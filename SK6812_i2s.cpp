...
// to his work on his software for driving SK6812 led-strips using
...
void SK6812::init(uint16_t _num_leds)
{
  uint8_t i;
  uint16_t j;
  ...
  for(i=0; i<SK6812_DITHER_NUM; i++)
  {
    ...
    {
      Serial.println("SK6812_I2S : ERROR ALLOCATING MEMORY");
      return;
    }
  ...
  for(i=0; i<SK6812_DITHER_NUM; i++)
  {
    ...
    if (i == (SK6812_DITHER_NUM-1)) // last DMA descriptor in linked list ?
    {
...
// Send out SK6812 bits with coded pulses, one nibble, then the other.
...
// display the pixels
void SK6812::show(pixel *pixels)
{
  uint8_t *buffer;
  uint8_t pixelbyte;
  uint8_t gammabyte;
  uint16_t i,b;
  uint16_t *i2s_ptr[SK6812_DITHER_NUM];
  ...
  // - get the pixel value (either R,G,B or W)
  // - for every dithered buffer
  //   get the gamma-corrected output value
  // - and transform into i2s nibble

  for(b=0; b<NUM_RGBW_BYTES; b++)
  {
    pixelbyte = double(*buffer++)*((double)brt/255.0);
...
void SK6812::setBrightness(uint8_t brt)
{
    this->brt = brt;
}
